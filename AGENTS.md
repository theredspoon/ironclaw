...
- **Integration-first for Reborn:** production-wired Reborn behavior ships with a `tests/integration/` test asserting at a seam (not `wait_for_status(Completed)` alone); crate-tier only when that tier can't reach the path (say why in the PR); never add test-only wiring for paths production doesn't wire; no `#[ignore]`d tests. Full rule: `.claude/rules/testing.md` ("Integration-First Coverage").
