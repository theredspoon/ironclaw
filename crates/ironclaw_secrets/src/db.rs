...
async fn libsql_upsert_session(
    ...
    conn.execute(
        ...
        libsql::params![
            ...
            key.thread_id,
            key.invocation_id,
            session.correlation_id().expose_for_storage(),
...
async fn libsql_get_session_record(
    ...
    let mut rows = conn
        .query(
            ...
            libsql::params![key.tenant_id, key.user_id, key.agent_id, key.project_id, key.mission_id, key.thread_id, key.invocation_id, session_id.expose_for_storage()],
        )
        .await
        .map_err(db_error)?;
    let Some(row) = rows.next().await.map_err(db_error)? else {
...
impl From<&CredentialSession> for PersistedCredentialSession {
    fn from(session: &CredentialSession) -> Self {
        Self {
            ...
            correlation_id: session.correlation_id.expose_for_storage(),
        }
    }
}
...
fn credential_session_aad(scope: &ResourceScope, session_id: CredentialSessionId) -> Vec<u8> {
    let key = DbScopeKey::from_full_scope(scope);
    let session_id_string = session_id.expose_for_storage();
    build_aad(
