...
use crate::postgres_backend::hashing::{Digest, invocation_key_hash, scope_hash, value_key_hash};
use crate::postgres_backend::schema::{INVOCATIONS_TABLE, VALUES_TABLE};
...
impl PostgresPredicateStateBackend {
    ...
    pub async fn run_migrations(&self) -> Result<(), PredicateBackendError> {
        ...
        client
            .batch_execute(crate::postgres_backend::schema::POSTGRES_PREDICATE_SCHEMA)
            .await
            .map_err(map_pg)?;
        Ok(())
    }
    ...
    /// **canonical** [`crate::predicate_state::window_cutoff`] (the
