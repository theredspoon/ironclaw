...
use ironclaw_matrix_adapter::{
    EncryptionState, MatrixParseInput, MatrixReasonCode, MatrixRenderInput, MatrixRouteMetadata,
    RelationKind, parse_matrix_event, render_matrix_outbound,
};
use ironclaw_product_adapters::{
    ExternalActorRef, ExternalConversationRef, FinalReplyView, ProductAttachmentKind,
...
fn preserves_reply_thread_and_dedup_metadata() {
    ...
    assert_eq!(parsed.facts.deduplication_key, "$event:example.org");
    assert_eq!(
...
fn render_rejects_unsupported_payloads_and_target_rooms() {
    ...
    let err = render_matrix_outbound(MatrixRenderInput {
        ...
        route_metadata: MatrixRouteMetadata {
            room_id: "!evil:example.org".to_string(),
            reply_to_event_id: None,
            thread_root_event_id: None,
        },
        allowed_target_rooms: vec!["!room:example.org".to_string()],
