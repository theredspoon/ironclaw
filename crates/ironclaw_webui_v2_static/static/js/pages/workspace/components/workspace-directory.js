...
export function WorkspaceDirectory({ path, entries, isLoading, filter, onOpen, onNavigate }) {
  ...
  let body;
  if (!visible.length) {
    body = html`<div className="px-4 py-10 text-center text-sm text-iron-300">${t("workspace.emptyDir")}</div>`;
  } else if (!rows.length) {
    body = html`<div className="px-4 py-10 text-center text-sm text-iron-300">${t("workspace.noMatches")}</div>`;
  } else {
    body = html`
      <div className="divide-y divide-white/[0.06]">
        ${rows.map((entry) => html`
          ...
          >
            ...
            <span className="min-w-0 truncate ${entry.is_dir ? "font-semibold" : ""}">${entry.name}</span>
          </button>
        `)}
      </div>
    `;
