...
<Info>
This is an operator-focused reference, not an exhaustive inventory of every internal, test-only, experimental, or provider-registry variable. When changing configuration behavior, treat the code in `src/config/`, `providers.json`, and the setup specification in `src/setup/README.md` as the source of truth.
</Info>
