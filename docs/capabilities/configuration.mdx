...
| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| `WEBHOOK_HOST` | string | `127.0.0.1` | Unified webhook listener bind host |
| `WEBHOOK_PORT` | int | `8080` | Unified webhook listener port |
| `HTTP_ENABLED` | bool | `false` | Enable the named HTTP webhook channel |
| `HTTP_HOST` | string | `127.0.0.1` | Legacy listener fallback and HTTP channel host |
| `HTTP_PORT` | int | `8080` | Legacy listener fallback and HTTP channel port |
| `HTTP_WEBHOOK_SECRET` | string | — | Shared secret for validation |

<Warning>
The unified webhook listener binds to `127.0.0.1:8080` by default. To receive webhooks from external services, set `WEBHOOK_HOST=0.0.0.0`.
</Warning>
