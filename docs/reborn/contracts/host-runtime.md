...
Surface versioning hashes the configured base version, `SurfaceKind`, visibility policy, visible capability ids/providers/runtimes/effects/schemas, selected resource estimates, and visible access status. The hash is stable across process runs so upper loop services can checkpoint the exact rendered tool surface. Direct invocation remains authoritative: a capability omitted from the visible surface must still fail closed through `CapabilityHost` authorization if a caller tries to invoke it directly.
