...
| Extension install/activate/remove | Local-dev composition wires extension search/install/activate/remove through `LifecycleProductFacade`; the Reborn CLI now exposes that facade for operator testing. | Extension lifecycle/config/package services behind facade. | Local-dev wired; production lifecycle store/composition still follow-up. |
