...
| WS-14 | PlannedDriver registration + run-profile selection | `ironclaw_reborn` | [`planned-driver-registration.md`](agent-loop-briefs/planned-driver-registration.md) | Register `default_planned_driver()` in `DriverRegistry` under `reborn:planned-default` v1 with checkpoint schema `reborn:default-loop-v1` v1; add the `reborn-planned-default` profile to the resolver. **Hard-gated on WS-9/11/12/13** — merge criterion is a real-host smoke test composing all four parallel adapters with no mocks. First end-to-end run through the new framework. |
...
**Minimum E2E set:** WS-9 + WS-10 + WS-11 + WS-12 + WS-13 + WS-14 are the smallest combination that lets a real turn run through the framework end-to-end. WS-14's smoke test is the merge gate — it hard-depends on all five parallel siblings because it composes their real adapters with no mocks, including WS-10's `load_checkpoint_payload` in the cancellation case (see the WS-14 brief §5). WS-15 surfaces identity-file context (the prompt is functional without it, but profile-specific behavior — `AGENTS.md`, `SOUL.md`, etc. — won't reach the model until it lands).

**Sequencing:** WS-9, WS-10, WS-11, WS-12, WS-13, and WS-15 run in parallel after the skeleton (WS-0..WS-8) lands; none of those six depend on each other. WS-14 is **hard-gated** on WS-9/10/11/12/13: its merge criterion is a real-host smoke test that composes the five parallel adapters with no mocks (the cancellation smoke specifically asserts on WS-10's `load_checkpoint_payload`), so WS-14 sits one level below the parallel band. WS-15 is independent of WS-14 (`identity_messages = Vec::new()` is a valid surface for the smoke test). All seven follow-up briefs now live under [`agent-loop-briefs/`](agent-loop-briefs/) — pre-written rather than scoped-at-pickup — because each answers a concrete external question (profile-scoped tool surface, identity-file → prompt wiring, durable checkpoint shape, queue substrate seam, milestone surface additions, sync-snapshot cancellation, driver registration) whose answers are stable enough to commit before implementation begins.
