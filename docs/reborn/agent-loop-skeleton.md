...
| WS-10 | Checkpoint store + resume path | `ironclaw_turns` (trait extension) + a checkpoint store crate | _scope at pickup_ | Add `load_checkpoint_payload(checkpoint_id) -> Vec<u8>` to `LoopCheckpointPort`. Implement durable backing store for `checkpoint` writes. Wire `PlannedDriver::resume` against the load path. Resume from `Blocked` actually works. |
| WS-11 | LoopInputPort implementation | `ironclaw_loop_support` | _scope at pickup_ | Implement `poll_inputs` + `ack_inputs` against host steering/followup queues. Steering messages reach the model mid-loop; followup messages restart the loop after a natural stop. |
| WS-12 | LoopProgressPort wiring | `ironclaw_loop_support` + `ironclaw_reborn` | _scope at pickup_ | Implement `emit_loop_progress` against the engine event substrate (per [`contracts/events.md`](contracts/events.md)). Executor emits milestones at strategy boundaries; SSE/audit observers see loop progress. |
| WS-13 | Cancellation accessor on AgentLoopDriverHost | `ironclaw_turns` (trait extension) + `ironclaw_loop_support` | _scope at pickup_ | Add the cancellation-observation method WS-6 §3.5 documents but never lands. Without this the executor cannot honor mid-loop cancellation. |
| WS-14 | PlannedDriver registration + run-profile selection | `ironclaw_reborn` | _scope at pickup_ | Register `default_planned_driver()` in `DriverRegistry` under a stable `LoopDriverId`. Update run-profile resolver so a profile can select `PlannedDriver` instead of `TextOnlyModelReplyDriver`. First end-to-end run through the new framework. |
| WS-15 | Prompt context assembly: identity-file surface | `ironclaw_loop_support` + `ironclaw_reborn` | [`prompt-context-assembly.md`](agent-loop-briefs/prompt-context-assembly.md) | Add `HostIdentityContextSource` trait (analogous to existing `HostSkillContextSource`). Wire it through `ThreadBackedLoopContextPort::load_loop_context()` so `LoopContextBundle.identity_messages` is populated from workspace identity files (`AGENTS.md`, `SOUL.md`, `USER.md`, `IDENTITY.md`, `HEARTBEAT.md`, `TOOLS.md`, `BOOTSTRAP.md`). Filename canon shared with `ironclaw_memory::safety::DEFAULT_PROMPT_PROTECTED_PATHS`. |

**Minimum E2E set:** WS-9 + WS-13 + WS-14 are the smallest combination that lets a real turn run through the framework end-to-end (capability execution + cancellation + driver routing). WS-10 adds resume; WS-11 adds steering UX; WS-12 adds observability; WS-15 surfaces identity-file context (the prompt is functional without it, but profile-specific behavior — `AGENTS.md`, `SOUL.md`, etc. — won't reach the model until it lands).

**Sequencing:** all seven follow-ups can run in parallel after the skeleton (WS-0..WS-8) lands. None depend on each other directly. WS-9 and WS-13 unblock the integration smoke tests against the *real* host (not just `MockAgentLoopDriverHost`). WS-9 and WS-15 are the two follow-ups whose design is pre-written in `agent-loop-briefs/` because both answer concrete external questions (profile-scoped tool surface; identity-file → prompt wiring) that come up before pickup-time scoping is comfortable.
