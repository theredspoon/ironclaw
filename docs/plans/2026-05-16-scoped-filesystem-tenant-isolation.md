...
| `ironclaw_threads` | `LibSqlSessionThreadService`, `PostgresSessionThreadService` | **Deleted** in `9a8aacab1` — `FilesystemSessionThreadService` is the sole impl; the `libsql`/`postgres` features on `ironclaw_threads` are gone. The `libsql-restart-tests` regression in `crates/ironclaw_runner/tests/loop_driver_host.rs` constructs the filesystem service over a `LibSqlRootFilesystem`. |
